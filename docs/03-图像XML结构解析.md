...
| 声明了非 UTF-8 | `encoding="gb18030"` 等 | 内容确认为纯 ASCII → 把声明改写成 `UTF-8` 并记 `NormalizedEncoding`；含多字节 → 返回 `ErrNonUTF8Content`，**绝不按 UTF-8 猜** | `internal/wzxml/repair.go:14` |
...
2. 需要**检索**它 → 改 `flattenScalars`（`internal/extract/extract.go:259`）的扁平化规则，注意 §8 的键名冲突。
...
由此产生的两个约束（`internal/store/store.go:845` `jsonPath` 必须处理）：
